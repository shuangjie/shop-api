...
func DeleteBrand(ctx *gin.Context) {
	...
	_, err = global.GoodsSrvClient.DeleteBrand(context.Background(), &proto.BrandRequest{Id: int32(brandId)})
	if err != nil {
		api.HandlerGrpcErrorToHttp(err, ctx)
		return
	}
...
func DeleteCategoryBrand(ctx *gin.Context) {
	...
	_, err = global.GoodsSrvClient.DeleteCategoryBrand(context.Background(), &proto.CategoryBrandRequest{Id: int32(brandId)})
	if err != nil {
		api.HandlerGrpcErrorToHttp(err, ctx)
		return
	}
