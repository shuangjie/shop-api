...
import (
	...
	validator2 "shop-api/order-web/validator"
)
...
func main() {
	...
	zap.S().Debugf("order-web服务启动中..., 端口: %d", global.ServerConfig.Port)
	go func() {
		if err := router.Run(fmt.Sprintf(":%d", global.ServerConfig.Port)); err != nil {
			zap.S().Panic("order-web服务启动失败", err.Error())
		}
	}()
